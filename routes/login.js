...
exports.post = function(req, res){
	...
	var authUrl= config.envInfo.url;
	console.log("authUrl: " + authUrl);
	var authPath = '/oauth2/v1/token';
	var authPort = 443;
	var authMethod = 'POST';
	...
	var options = {
		...
		headers: { 'Content-Length': body.length, 'Content-Type': 'application/x-www-form-urlencoded','Authorization': config.envInfo.basicToken }
	};
	...
	var auth_req = https.request(options, function(auth_res) {
		auth_res.setEncoding('utf8');
	  	auth_res.on('data', function (data) {
            ...
            console.log(qs.decode(req.query));
	  	  	// Collect data from request
	  	  	var client_id = req.query.client_id;
	  	  	var scope  = req.query.scope;
	  	  	var state  = req.query.state;
	  	  	var appName = req.query.appName; //**CHANGE THIS
			var redirect_uri = req.query.redirect_uri;
			var basePath = utils.getBasePath(req);
...
/*					res.statusCode = 302;
	  	  		...
	  	  		var regLink  = 'http://'+config.envInfo.host+config.envInfo.port+'/register?';
